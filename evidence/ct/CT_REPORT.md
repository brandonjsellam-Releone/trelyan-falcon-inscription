...
1. **At the pre-registered lines, key-dependence of signing time is not detected — at this power:** all three message-balanced fixed-key pairs and the pool-vs-pool control PASS on the raw statistic (|t| ≤ 1.04, every CI spanning zero) **and** on the crop diagnostic against a null of the same kind (crop *p*ₑₘₚ 0.10–0.29; with N = 20 the empirical *p* has resolution 1/21). The v3 rule's word is **PASS**, and PASS means exactly: *the pre-registered detection conditions were not met at this power on this machine and build; the design is insensitive to effects of order ±15 µs and bounds them only within the reported CIs (±25–60 µs); not a proof of absence.* Sign convention: raw *t* is (mean₀ − mean₁)/se while Δmean is mean₁ − mean₀, so their signs are opposite by construction.
...
3. **Power caveat, stated plainly:** the four-message rotation widens each class into a mixture (sd 0.41–1.06 ms here vs 0.21–0.23 ms in v2), so 95 % CIs on Δmean are ≈ ±25–60 µs versus ±10 µs in v2. **The ~12–17 µs fixed-key deltas v2 recorded are therefore neither confirmed nor excluded by this session.** Resolving effects of that size under v3's design needs far more data — a back-of-envelope estimate at v3's variance (sd ≈ 0.7 ms; se_Δ = sd·√(2/n) ≤ 5 µs) gives ≈ 40 k per class, i.e. **≈ 40–80 k measurements per experiment** — an estimate, not a pre-specified power calculation; the next run keeps the same rules and states its target power before running.
...
**Consequences:** Phase E stays **blocked** — one PASS on one machine at limited power is not "a sound PASS exists"; METHODOLOGY says a PASS is not a proof, and the v2 deltas are unresolved. What this session adds: after three pre-registered iterations, the pinned det1024 signer's timing shows **no detectable location dependence on the key at the tested power** (the design cannot see ±15 µs); the earlier FAIL (v1) and SHAPE (v2) readings are non-evidentiary under the corrected statistic and null; the open question is sized (effects of order 15 µs, ≈ 40–80 k measurements per experiment to resolve at v3's variance). No patch to the vendored primitive; nothing here is a security claim in either direction.
