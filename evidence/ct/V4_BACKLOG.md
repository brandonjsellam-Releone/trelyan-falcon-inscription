...
- **Environment control is cheaper than samples.** The binding constraint on resolution is the
  sd, which spans **2.4×** across pairs *within a single session* (425–1 039 µs). Core pinning
  and a quiet box would buy more than quadrupling the measurement count, which costs 4× the
  wall-clock for 2× the resolution. `affinity_pinned: false` is honestly reported today; a
  dependency-free approach (or an explicitly justified dependency) is worth pricing.
...
4. **Replace the fixed 4.5 gate on null sessions with a null-referenced threshold.** The
   pool-vs-pool null is a random-effects null: `sd(t_null)² ≈ 1 + (σ_key/σ_tot)²·(n/32)`, which
   grows with *n*. At ~10× the v3.1 samples the gate starts failing arithmetically rather than
   environmentally. **Decide before the next size increase, not after a result.**
